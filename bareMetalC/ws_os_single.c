// ws_os_single.c  — One-shot Gemmini MM (WS or OS) with optional warmup
...
// ===== Core kernel (config + matmul; 측정은 여기서만) =====
...
int main(void){
  ...
  // 3) 웜업 (측정 제외). 기본 1회. 웜업 후에는 flush/fence로 상태를 지우지 않음(예열 유지)
  for (int w = 0; w < WARMUP_RUNS; w++) {
    (void)gemmini_mm_kernel(DATAFLOW, M, N, K, A_buf, B_buf, D_bias, C_out);
  }

  // 4) 측정 1회
  uint64_t cyc = gemmini_mm_kernel(DATAFLOW, M, N, K, A_buf, B_buf, D_bias, C_out);
